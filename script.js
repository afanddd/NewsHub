...
// Fungsi untuk menampilkan berita berdasarkan kategori, pencarian, dan halaman
function displayNews(category = "all", searchQuery = "", page = 1, append = false) {
    const newsContainer = document.getElementById('news-container');
    if (!append) {
        newsContainer.innerHTML = ""; // Kosongkan kontainer hanya jika tidak menambahkan berita baru
    }
    ...
    if (filteredNews.length === 0) {
        ...
        renderPagination(0); // Sembunyikan pagination jika tidak ada berita
        return;
    }

    // Tampilkan berita untuk halaman yang dipilih
    ...
    paginatedNews.forEach(news => {
        ...
        const newsImage = news.image ? `<img src="${news.image}" alt="${news.title}">` : '';
        const newsContent = `
            ${newsImage}
            <div class="content">
                <h2>${news.title}</h2>
...
function renderPagination(totalPages, currentPage = 1) {
    ...
    if (totalPages <= 1) return; // Sembunyikan pagination jika hanya ada 1 halaman
...
document.addEventListener('click', (e) => {
    ...
    // Jika yang diklik bukan menu atau tombol hamburger, tutup menu
...
function loadMoreNews() {
    const activeCategory = document.querySelector('nav ul li a.active')?.getAttribute('data-category') || "all";
    const searchQuery = document.getElementById('search-input').value;
    displayNews(activeCategory, searchQuery, currentPage, true); // Tambahkan berita baru tanpa menghapus yang lama
    isLoading = false;
}
